...
ReportRequest<PSCustomObject>
#>

function Initialize-LELEReportRequest {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSLoginEnterprise => LEReportRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $ReportPeriodStart) {
            throw "invalid value for 'ReportPeriodStart', 'ReportPeriodStart' cannot be null."
        }
...
Convert from JSON to ReportRequest<PSCustomObject>
...
ReportRequest<PSCustomObject>
#>
function ConvertFrom-LEJsonToReportRequest {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSLoginEnterprise => LEReportRequest' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in LEReportRequest
        $AllProperties = ("reportPeriodStart")
        ...
        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'reportPeriodStart' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "reportPeriodStart"))) {
            throw "Error! JSON cannot be serialized due to the required property 'reportPeriodStart' missing."
        } else {
