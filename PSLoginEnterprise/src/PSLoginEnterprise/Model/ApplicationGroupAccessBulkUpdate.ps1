...
.PARAMETER ApplicationGroupIds
Application group IDs to update
.PARAMETER RoleIds
...
function Initialize-LEApplicationGroupAccessBulkUpdate {
    [CmdletBinding()]
    Param (
...
Convert from JSON to ApplicationGroupAccessBulkUpdate<PSCustomObject>
...
function ConvertFrom-LEJsonToApplicationGroupAccessBulkUpdate {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "applicationGroupIds"))) { #optional property not found
            $ApplicationGroupIds = $null
        } else {
            $ApplicationGroupIds = $JsonParameters.PSobject.Properties["applicationGroupIds"].value
        }
