...
Environment data update from provider result

.PARAMETER Successful
True when successfully updated environment data from provider
.PARAMETER WarningMessage
Warning message - null when operation was successful
.OUTPUTS

UpdateFromProviderResult<PSCustomObject>
#>

function Initialize-LEUpdateFromProviderResult {
    [CmdletBinding()]
    Param (
...
Convert from JSON to UpdateFromProviderResult<PSCustomObject>
...
function ConvertFrom-LEJsonToUpdateFromProviderResult {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "warningMessage"))) { #optional property not found
            $WarningMessage = $null
        } else {
            $WarningMessage = $JsonParameters.PSobject.Properties["warningMessage"].value
        }
