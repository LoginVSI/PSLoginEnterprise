...
No description available.
.PARAMETER SeamlessMode
Enable or disable Seamless mode for published applications
.OUTPUTS

NetscalerConnector<PSCustomObject>
#>

function Initialize-LELENetscalerConnector {
    [CmdletBinding()]
    Param (
...
Convert from JSON to NetscalerConnector<PSCustomObject>
...
function ConvertFrom-LEJsonToNetscalerConnector {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "seamlessMode"))) { #optional property not found
            $SeamlessMode = $null
        } else {
            $SeamlessMode = $JsonParameters.PSobject.Properties["seamlessMode"].value
        }
