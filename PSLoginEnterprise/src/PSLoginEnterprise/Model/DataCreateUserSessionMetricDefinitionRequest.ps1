...
User session metric definition creation data
...
function ConvertFrom-LEJsonToDataCreateUserSessionMetricDefinitionRequest {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "PerformanceCounterDefinitionCreate"
                    $match++
                    break
                }
            }
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "WmiQueryDefinitionCreate"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([PerformanceCounterDefinitionCreate, WmiQueryDefinitionCreate]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("PerformanceCounterDefinitionCreate", "WmiQueryDefinitionCreate")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([PerformanceCounterDefinitionCreate, WmiQueryDefinitionCreate]). JSON Payload: $($Json)"
        }
    }
}
