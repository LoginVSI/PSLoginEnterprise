...
function ConvertFrom-LEJsonToConfigurationGetAccountGroup200Response {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "AccountSelectionGroup"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([AccountFilterGroup, AccountSelectionGroup]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("AccountFilterGroup", "AccountSelectionGroup")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([AccountFilterGroup, AccountSelectionGroup]). JSON Payload: $($Json)"
        }
    }
}
