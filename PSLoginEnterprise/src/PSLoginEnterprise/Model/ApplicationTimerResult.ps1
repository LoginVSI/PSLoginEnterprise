...
function Initialize-LELEApplicationTimerResult {
    [CmdletBinding()]
    Param (
...
Convert from JSON to ApplicationTimerResult<PSCustomObject>
...
function ConvertFrom-LEJsonToApplicationTimerResult {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "performanceResult"))) { #optional property not found
            $PerformanceResult = $null
        } else {
            $PerformanceResult = $JsonParameters.PSobject.Properties["performanceResult"].value
        }
