...
function Initialize-HorizonConnector {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSLoginEnterprise => HorizonConnector' | Write-Debug
        ...
        if ($ServerUrl.length -lt 1) {
            throw "invalid value for 'ServerUrl', the character length must be greater than or equal to 1."
        }
        ...
        if ($Resource.length -lt 1) {
            throw "invalid value for 'Resource', the character length must be greater than or equal to 1."
        }
        ...
        if ($CommandLine.length -lt 1) {
            throw "invalid value for 'CommandLine', the character length must be greater than or equal to 1."
        }
...
HorizonConnector<PSCustomObject>
#>
function ConvertFrom-JsonToHorizonConnector {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSLoginEnterprise => HorizonConnector' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in HorizonConnector
