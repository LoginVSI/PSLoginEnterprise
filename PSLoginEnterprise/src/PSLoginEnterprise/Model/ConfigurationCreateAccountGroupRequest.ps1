...
function ConvertFrom-LEJsonToConfigurationCreateAccountGroupRequest {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "AccountFilterGroupCreate"
                    $match++
                    break
                }
            }
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "AccountSelectionGroupCreate"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([AccountFilterGroupCreate, AccountSelectionGroupCreate]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("AccountFilterGroupCreate", "AccountSelectionGroupCreate")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([AccountFilterGroupCreate, AccountSelectionGroupCreate]). JSON Payload: $($Json)"
        }
    }
}
