...
User session metric definition
...
function ConvertFrom-LEJsonToDataGetUserSessionMetricDefinitionByKey200Response {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "BuiltInMetricDefinition"
                    $match++
                    break
                }
            }
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "PerformanceCounterDefinition"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([BuiltInMetricDefinition, PerformanceCounterDefinition, WmiQueryDefinition]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("BuiltInMetricDefinition", "PerformanceCounterDefinition", "WmiQueryDefinition")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([BuiltInMetricDefinition, PerformanceCounterDefinition, WmiQueryDefinition]). JSON Payload: $($Json)"
        }
    }
}
