...
#>
function Invoke-DataAddPlatformMetrics {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Calling method: Invoke-DataAddPlatformMetrics' | Write-Debug
        ...
        $LocalVarUri = '/v7/platform-metrics'

        if (!$PlatformMetric) {
            throw "Error! The required parameter `PlatformMetric` missing when calling dataAddPlatformMetrics."
        }

        $LocalVarBodyParameter = ConvertTo-Json @($PlatformMetric) -Depth 100
...
#>
function Invoke-DataGetPlatformMetrics {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipeline = $true, ValueFromPipelineByPropertyName = $true, Mandatory = $false)]
    ...
    Process {
        'Calling method: Invoke-DataGetPlatformMetrics' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug
