...
.PARAMETER EnableEuxRawDataSave
Enable EUX raw data save
.OUTPUTS

EuxSettings<PSCustomObject>
#>

function Initialize-LEEuxSettings {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [System.Nullable[Boolean]]
        ${EnableEuxRawDataSave}
    )
    ...
    Process {
        ...
        $PSO = [PSCustomObject]@{
            "enableEuxRawDataSave" = ${EnableEuxRawDataSave}
        }
...
Convert from JSON to EuxSettings<PSCustomObject>
...
function ConvertFrom-LEJsonToEuxSettings {
    ...
    Process {
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "enableEuxRawDataSave"))) { #optional property not found
            $EnableEuxRawDataSave = $null
        } else {
            $EnableEuxRawDataSave = $JsonParameters.PSobject.Properties["enableEuxRawDataSave"].value
        }
