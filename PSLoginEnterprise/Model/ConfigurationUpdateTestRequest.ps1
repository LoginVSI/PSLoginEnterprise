...
function ConvertFrom-LEJsonToConfigurationUpdateTestRequest {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "ApplicationTestUpdate"
                    $match++
                    break
                }
            }
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "ContinuousTestUpdate"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([ApplicationTestUpdate, ContinuousTestUpdate, LoadTestUpdate]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("ApplicationTestUpdate", "ContinuousTestUpdate", "LoadTestUpdate")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([ApplicationTestUpdate, ContinuousTestUpdate, LoadTestUpdate]). JSON Payload: $($Json)"
        }
    }
}
