...
function ConvertFrom-LEJsonToConfigurationCreateApplicationRequest {
    ...
    Process {
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "WebApplicationCreate"
                    $match++
                    break
                }
            }
        ...
        try {
            ...
            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "WindowsApplicationCreate"
                    $match++
                    break
                }
            }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([WebApplicationCreate, WindowsApplicationCreate]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("WebApplicationCreate", "WindowsApplicationCreate")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([WebApplicationCreate, WindowsApplicationCreate]). JSON Payload: $($Json)"
        }
    }
}
