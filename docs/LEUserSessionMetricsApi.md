# PSLoginEnterprise.PSLoginEnterprise\Api.LEUserSessionMetricsApi
...
# Get list of user session metrics
try {
    $Result = Get-LEUserSessionMetrics -TestRunId $TestRunId -From $From -To $To -OrderBy $OrderBy -Direction $Direction -Count $Count -Offset $Offset -IncludeTotalCount $IncludeTotalCount
} catch {
